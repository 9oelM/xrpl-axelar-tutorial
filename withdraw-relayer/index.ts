...
function getConfig(): Config {
  ...
  const port = process.env.WITHDRAW_RELAYER_PORT
    ? parseInt(process.env.WITHDRAW_RELAYER_PORT, 10)
    : 3000;
  ...
  if (!privateKey)
    throw new Error(
      "WITHDRAW_RELAYER_PRIVATE_KEY environment variable is required",
    );
  if (!contractAddress)
...
async function checkApprovalStatus() {
  try {
    const allowance = await xrpContract.allowance(
      wallet.address,
      config.contractAddress,
    );
...
approvalPromise = (async () => {
  try {
    ...
    const tx = await xrpContract.approve(
      config.contractAddress,
    ...
    if (await checkApprovalStatus()) {
      ...
      throw new Error(
        "Approval transaction completed but allowance not updated",
      );
    }
  } catch (error) {
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "Missing required parameters: withdrawAccount, publicKey, requestedAmount, timestamp, signature",
      });
    }
    ...
    if (isNaN(Number(requestedAmount)) || Number(requestedAmount) <= 0) {
      return res.status(400).json({
        error:
          "Invalid requestedAmount. It must be a positive number in string format, in the unit of native XRP.",
      });
    }
    ...
    const amount = dropsToEVMSidechainXRPDecimals(
      BigInt(xrpl.xrpToDrops(requestedAmount)),
    );
